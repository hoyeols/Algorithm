...
"이름"열의 내용이 실제 문제를 푸는 과정에 필요하지는 않습니다. "이름"열에는 서로 다른 이름들이 중복없이 채워져 있다고 가정하고 문제를 해결해 주세요.
...
def solution(n, k, cmd):
	...
	if len(C_idx) == 1:
		for idx in C_idx:
			for c in range(idx):
			#C와 Z를 제외한 명령어 방향과 이동수 저장
				if cmd[c] == 'C' or cmd[c] == 'Z':
					continue
				arrow, amount = cmd[c].split(' ')
		...
		for idx in C_idx:
			...
			#C와 Z를 제외한 명령어 방향과 이동수 저장
				...
				if cmd[c] == 'C' or cmd[c] == 'Z' or c not in C_idx:
					continue
